...
double logSURLikelihood(const arma::mat& data, const arma::uvec& outcomesIdx,
  ...
  double a_r_n, b_r_n;
  double logP, sign,
      tmp;  // sign is needed for the implementation, but we 'assume' that all
            // the matrices are (semi-)positive-definite (-> det>=0)
  // tmp is a temporary container for the log determinants needed

  arma::mat XtX;

  logP = -log(M_PI) * ((double)n * (double)s *
                       0.5);  // initialise with the normalising constant
                              // remaining from the likelhood

#ifdef _OPENMP
  omp_set_lock(&RNGlock);  // for some reason now I need to set a lock here or
                           // other threads will interfere....
#endif
  ...
  for (unsigned int l = 0; l < s; ++l)  // for each univariate outcome ..
  {
    ...
    xVS_IN = arma::join_vert(
        arma::regspace<arma::uvec>(
            0, fixedPredictorsIdx.n_elem - 1),                 // the fixed part
    ...
    // if ( !debug.is_sympd() ){
    // 	std::cout << "Warning: THIS IS NOT SYMPD!" << std::endl << debug <<
    // std::endl << std::endl;

    // 	std::cout << "VS_IN: " << VS_IN.t() << std::endl << "xVS_IN: " <<
    // xVS_IN.t() << std::endl << std::endl;

    // 	std::cout << "Data Matrix: " << std::endl << data.cols(VS_IN) <<
    // std::endl << std::endl; 	char c; std::cin >> c;
    // }

    W_n = arma::inv_sympd(XtX + arma::inv_sympd(W_0(xVS_IN, xVS_IN)));
    tilde_B = W_n * (arma::trans(data.cols(VS_IN)) *
                     data.col(outcomesIdx(l)) /* + W_0.i() * ZERO  */);

    a_r_n = a_r_0 + 0.5 * (double)n;
    b_r_n = b_r_0 + 0.5 * arma::as_scalar(
                              arma::trans(data.col(outcomesIdx(l))) *
                                  data.col(outcomesIdx(l)) -
...
void banditProposal(arma::umat& gamma, arma::mat& zeta, arma::umat& gamma_prop,
  ...
  if (Distributions::randU01() < 0.5)  // one deterministic update
  {
    ...
    update_idx = Distributions::randWeightedIndexSampleWithoutReplacement(
        p * s, normalised_mismatch);  // sample the one

    // Update
    gamma_prop(update_idx) =
        1 - gamma(update_idx);  // deterministic, just switch

    // Compute log_proposal probabilities
    normalised_mismatch_backwards = mismatch;
    normalised_mismatch_backwards(update_idx) =
        1. - normalised_mismatch_backwards(update_idx);
    // normalised_mismatch_backwards = normalised_mismatch_backwards -
    // Utils::logspace_add(normalised_mismatch_backwards);
    normalised_mismatch_backwards = normalised_mismatch_backwards /
                                    arma::sum(normalised_mismatch_backwards);

    log_proposal =
        arma::as_scalar(arma::log(normalised_mismatch_backwards(update_idx))) -
    ...
    update_idx = Distributions::randWeightedIndexSampleWithoutReplacement(
        p * s, normalised_mismatch, nUpdates);  // sample nUpdates indexes

    normalised_mismatch_backwards = mismatch;  // copy for backward proposal

    // Update
    for (unsigned int i = 0; i < nUpdates; ++i) {
      gamma_prop(update_idx(i)) =
          Distributions::randBernoulli(zeta(update_idx(i)));  // random update

      if (gamma_prop(update_idx(i)) != gamma(update_idx(i))) {
        normalised_mismatch_backwards(update_idx(i)) =
            1. - normalised_mismatch_backwards(update_idx(i));
        log_proposal += Distributions::logPDFBernoulli(gamma(update_idx(i)),
                                                       zeta(update_idx(i))) -
                        Distributions::logPDFBernoulli(
                            gamma_prop(update_idx(i)), zeta(update_idx(i)));
      }
    }
    ...
    log_proposal += Distributions::logPDFWeightedIndexSampleWithoutReplacement(
                        normalised_mismatch_backwards, update_idx) -
                    Distributions::logPDFWeightedIndexSampleWithoutReplacement(
                        normalised_mismatch, update_idx);
  }

  // A/R step

  double logLik_prop =
      logSURLikelihood(data, outcomesIdx, fixedPredictorsIdx, vsPredictorsIdx,
                       gamma_prop, a_r_0, b_r_0, W_0, temp);

  double logPrior_prop = logSURPrior(omega_curr, gamma, a_0, b_0);

  double logAccProb = (logLik_prop - logLik_curr) +
                      (logPrior_prop - logPrior_curr) + log_proposal;
  ...
  for (arma::uvec::iterator iter = update_idx.begin(); iter != update_idx.end();
       ++iter) {
    if (finiteAdaptation) {
...
void bandit_SUR_MCMC_step(
  ...
  predUpdate = arma::join_rows(
      Distributions::randIntUniform(nUpdates, 0, p - 1),
      Distributions::randIntUniform(nUpdates, 0,
                                    s - 1));  // note that I might be updating
                                              // multiple times the same coeff

  double a, b;

  // ## Update Omega
  // FULL CONDITIONAL (no sense in making it parallel for now as the RNG is
  // still sequential) note that a_0 and b_0 are vectors of size p, meaning
  // there's one prior for each beta, regardless of the associated Y
  for (unsigned int k = 0; k < s; ++k) {
    for (unsigned int j = 0; j < p; ++j) {
      a = a_0(j) + gamma_curr(j, k);
      b = b_0(j) + 1. - gamma_curr(j, k);
      omega_curr(j, k) =
          Distributions::randBeta((a + 1.) / temp - 1., b / temp);
    }
  }

  // Update the prior
  logPrior_curr = logSURPrior(omega_curr, gamma_curr, a_0,
                              b_0);  // update the prior probability

  // ## Update Gamma -- Local move
  banditProposal(gamma_curr, zeta, gamma_prop, alpha_z, beta_z, mismatch,
                 normalised_mismatch, normalised_mismatch_backwards, data,
                 outcomesIdx, fixedPredictorsIdx, vsPredictorsIdx, omega_curr,
                 logPrior_curr, logLik_curr, a_r_0, b_r_0, W_0, a_0, b_0,
...
void MC3_SUR_MCMC_step(const arma::mat& data, const arma::uvec& outcomesIdx,
  ...
  // unsigned int n = Y.n_rows;
  unsigned int p =
      gamma_curr.n_rows;  // note that p now is just the nmber of vsPredictors
                          // essentially, not of the whole set
  unsigned int s = outcomesIdx.n_elem;
  arma::imat predUpdate;  // which predictor are we updating?
  ...
  // predUpdate = arma::join_rows( Distributions::randIntUniform(nUpdates,
  // 0,p-1) , Distributions::randIntUniform(nUpdates,0,s-1) );    // note that I
  // might be updating multiple times the same coeff
  ...
  // ## Update Gamma -- Local move
  predUpdate = Distributions::randIntUniform(
      nUpdates, 0,
      p - 1);  // note that I might be updating multiple times the same coeff

  for (unsigned int j = 0; j < nUpdates; ++j) {
    gamma_prop = gamma_curr;

    // Decide which l (l=0,...s-1) to sample
    outUpdate = Distributions::randIntUniform(
        0,
        s - 1);  // maybe I need to update more often here that I have a matrix?

    /// Sample Gamma (see
    /// http://www.genetics.org/content/suppl/2011/09/16/genetics.111.131425.DC1/131425SI.pdf
    /// pag 5)
    gamma_prop(predUpdate(j), outUpdate) = Distributions::randBernoulli(
        pow(omega_curr(predUpdate(j), outUpdate), 1. / temp) /
        (pow(omega_curr(predUpdate(j), outUpdate), 1. / temp) +
         pow(1. - omega_curr(predUpdate(j), outUpdate), 1. / temp)));

    if (gamma_prop(predUpdate(j), outUpdate) !=
        gamma_curr(predUpdate(j), outUpdate)) {
      // Compute log Probability for the current point
      logLik_prop = logSURLikelihood(data, outcomesIdx, fixedPredictorsIdx,
                                     vsPredictorsIdx, gamma_prop, a_r_0, b_r_0,
                                     W_0, temp);
      ...
      if (Distributions::randLogU01() < logAccProb) {
        ...
        gamma_curr(predUpdate(j), outUpdate) =
            gamma_prop(predUpdate(j), outUpdate);
        logLik_curr = logLik_prop;
        logPrior_curr = logSURPrior(omega_curr, gamma_curr, a_0,
                                    b_0);  // update the prior probability
      }
...
void exchange_step(arma::ucube& gamma_state, arma::cube& omega_state,
  ...
  arma::mat omegaSwap;
  arma::umat
      gammaSwap;  // TODO, there's really no need to use a temp matrix when only
                  // a temp double is needed to swap in a loop...
  double logPriorSwap;
  double logLikSwap;

  // Select the chains to swap
  chainIdx = (nChains > 2) ? Distributions::randIntUniform(
                                 1, (nChains) * (nChains - 1) / 2)
                           : 1;  // (nChains-1)*(nChains-2)/2 is the number of
                                 // possible chain combinations with nChains
  ...
  // Swap probability
  pExchange = logLik_state(secondChain) *
                  (temperature(secondChain) / temperature(firstChain) -
                   1.) +  // the plus is correct, don't doubt
              logLik_state(firstChain) *
                  (temperature(firstChain) / temperature(secondChain) - 1.);

  // The prior doesn't come into play as it's not tempered and hence there's no
  // effect from it

  // DEBUG CHECK
  if (firstChain == 0 || secondChain == 0) {
    // std::cout << std::exp(pExchange) << std::endl <<std::flush;
    // std::cout << "Current -- " << logLik_state(firstChain)  << "  --  " <<
    // logLik_state(secondChain) << "  ===  " << logLik_state(firstChain) +
    // logLik_state(secondChain)  << std::endl <<std::flush; std::cout <<
    // "Proposed -- " <<
    ...
    // +
    // logLik_state(firstChain)*temperature(firstChain)/temperature(secondChain)
    // - logLik_state(firstChain) - logLik_state(secondChain) ) << std::endl
    // <<std::flush; std::cout << temperature(firstChain)  << "  --  " <<
    // temperature(secondChain) << std::endl <<std::flush; int jnk; std::cin >>
    // jnk; return ;
  }
  ...
  if (Distributions::randLogU01() < pExchange) {
    ...
    logLikSwap = logLik_state(secondChain) * temperature(secondChain) /
                 temperature(firstChain);
...
void nearExchange_step(arma::ucube& gamma_state, arma::cube& omega_state,
  ...
  if (nChains > 2) {
    firstChain = Distributions::randIntUniform(
        1, nChains - 2);  // so not the first (0) or last (nChains-1) indexes
    secondChain = (Distributions::randU01() < 0.5)
                      ? firstChain - 1
                      : firstChain + 1;  // then select a neighbour
  } else {
    // with just 2 chains
    firstChain = 0;
    secondChain = 1;
  ...
  // if( firstChain == 0 || secondChain == 0 )   // if the swap involves the
  // main chain
  countGlobalUpdates++;
...
void allExchange_step(arma::ucube& gamma_state, arma::cube& omega_state,
  ...
  indexTable(tabIndex, 0) = 0;
  indexTable(tabIndex, 1) = 0;
  tabIndex++;

  for (unsigned int c = 1; c < nChains; ++c) {
    for (unsigned int r = 0; r < c; ++r) {
      indexTable(tabIndex, 0) = r;
      indexTable(tabIndex, 1) = c;
      tabIndex++;
    }
  }
...
#pragma omp parallel for private(tabIndex, firstChain, secondChain)
  ...
  for (tabIndex = 1; tabIndex <= ((nChains) * (nChains - 1) / 2); ++tabIndex) {
    ...
    // Swap probability
    // pExchange(tabIndex) = logLik_state(firstChain) * temperature(firstChain)
    // / temperature(secondChain) + 	logLik_state(secondChain) *
    // temperature(secondChain) / temperature(firstChain) -
    // 	logLik_state(firstChain) - logLik_state(secondChain);

    pExchange(tabIndex) =
        logLik_state(secondChain) *
            (temperature(secondChain) / temperature(firstChain) -
             1.) +  // the plus is correct, don't doubt
        logLik_state(firstChain) *
            (temperature(firstChain) / temperature(secondChain) - 1.);
  ...
  arma::vec cumulPExchange = arma::cumsum(
      arma::exp(pExchange - logSumWeights));  // this should sum to one
...
void uniformCrossOver_step(
  ...
  for (unsigned int j = 0; j < p; ++j) {
    for (unsigned int l = 0; l < s; ++l) {
      if (Distributions::randU01() < 0.5) {
        ...
        gammaXO(j, l, 1) = gamma_state(
            j, l, secondChain);  // 1-gammaXO(j,l,0); why did I have this? TODO
        ...
        gammaXO(j, l, 1) = gamma_state(
            j, l, firstChain);  // 1-gammaXO(j,l,0); why did I have this? TODO
  ...
  logPriorFirst =
      logSURPrior(omega_state.slice(firstChain), gammaXO.slice(0), a_0, b_0);
  logPriorSecond =
      logSURPrior(omega_state.slice(secondChain), gammaXO.slice(1), a_0, b_0);

  logLikFirst = logSURLikelihood(data, outcomesIdx, fixedPredictorsIdx,
                                 vsPredictorsIdx, gammaXO.slice(0), a_r_0,
                                 b_r_0, W_0, temperature(firstChain));

  logLikSecond = logSURLikelihood(data, outcomesIdx, fixedPredictorsIdx,
                                  vsPredictorsIdx, gammaXO.slice(1), a_r_0,
                                  b_r_0, W_0, temperature(secondChain));

  pCrossOver = (logPriorFirst + logLikFirst - logPrior_state(firstChain) -
                logLik_state(firstChain)) +
               (logPriorSecond + logLikSecond - logPrior_state(secondChain) -
                logLik_state(secondChain));
...
void blockCrossOver_step(
  ...
  arma::uvec covIdx =
      arma::find(arma::abs(covariatesCorrelation.row(predIdx)) >
                 threshold);  // this will include the original predIdx
  ...
  for (unsigned int j = 0; j < covIdx.n_elem; ++j) {
    gammaXO(covIdx(j), outcIdx, 0) =
        gamma_state(covIdx(j), outcIdx, secondChain);
    gammaXO(covIdx(j), outcIdx, 1) =
        gamma_state(covIdx(j), outcIdx, firstChain);

    omegaXO(covIdx(j), outcIdx, 0) =
        omega_state(covIdx(j), outcIdx, secondChain);
    omegaXO(covIdx(j), outcIdx, 1) =
        omega_state(covIdx(j), outcIdx, firstChain);
  }
  ...
  pCrossOver += 0.;  // XO prop probability is weird, how do I compute it? Let's
                     // say is symmetric as is determnistic and both comes from
                     // the same covariatesCorrelation
...
void adapCrossOver_step(
  ...
  for (unsigned int j = 0; j < p; ++j) {
    for (unsigned int l = 0; l < s; ++l) {
      if (gamma_state(j, l, firstChain) == gamma_state(j, l, secondChain)) {
        ...
        gammaXO(j, l, 0) = (Distributions::randU01() < pXO_0)
                               ? 1 - gammaXO(j, l, 0)
                               : gammaXO(j, l, 0);
        gammaXO(j, l, 1) = (Distributions::randU01() < pXO_0)
                               ? 1 - gammaXO(j, l, 1)
                               : gammaXO(j, l, 1);
        ...
        gammaXO(j, l, 0) =
            gamma_state(j, l, firstChain);  // check correctness TODO
        gammaXO(j, l, 1) = gamma_state(j, l, secondChain);

        gammaXO(j, l, 0) = (Distributions::randU01() < pXO_1)
                               ? 1 - gammaXO(j, l, 0)
                               : gammaXO(j, l, 0);
        gammaXO(j, l, 1) = (Distributions::randU01() < pXO_2)
                               ? 1 - gammaXO(j, l, 1)
                               : gammaXO(j, l, 1);
  ...
  pCrossOver +=
      (n11 * log(p11) + n12 * log(p12) + n21 * log(p21) +
       n22 * log(p22)) -  // CrossOver proposal probability FORWARD
      (n11 * log(p11) + n12 * log(p21) + n21 * log(p12) +
       n22 * log(p22));  // XO prop probability backward (note that ns stays the
                         // same but changes associated prob)
...
void MCMC_Global_step(const arma::mat& data, const arma::uvec& outcomesIdx,
  ...
  for (unsigned int k = 0; k < nGlobalUpdates; ++k)  // repeat global updates
  {
    ...
    switch (globalType) {
      ...
      // -- Exchange
      case 1:
        Model::exchange_step(gamma_state, omega_state, logPrior_state,
                             logLik_state, temperature, nChains, nGlobalUpdates,
                             accCountGlobalUpdates, countGlobalUpdates);
        break;

      case 2:
        Model::nearExchange_step(
            gamma_state, omega_state, logPrior_state, logLik_state, temperature,
            nChains, nGlobalUpdates, accCountGlobalUpdates, countGlobalUpdates);
        break;

      case 3:
        Model::allExchange_step(
            gamma_state, omega_state, logPrior_state, logLik_state, temperature,
            nChains, nGlobalUpdates, accCountGlobalUpdates, countGlobalUpdates);
        break;
      ...
      // -- CrossOver
      case 4:
        Model::uniformCrossOver_step(
            gamma_state, omega_state, logPrior_state, logLik_state, data,
            outcomesIdx, fixedPredictorsIdx, vsPredictorsIdx, a_r_0, b_r_0, W_0,
            a_0, b_0, temperature, nChains, nGlobalUpdates,
            accCountGlobalUpdates, countGlobalUpdates);
        break;

      case 5:
        Model::adapCrossOver_step(
            gamma_state, omega_state, logPrior_state, logLik_state, data,
            outcomesIdx, fixedPredictorsIdx, vsPredictorsIdx, a_r_0, b_r_0, W_0,
            a_0, b_0, temperature, pXO_0, pXO_1, pXO_2, p11, p12, p21, p22,
            nChains, nGlobalUpdates, accCountGlobalUpdates, countGlobalUpdates);
        break;

      case 6:
        Model::blockCrossOver_step(
            gamma_state, omega_state, logPrior_state, logLik_state, data,
            outcomesIdx, fixedPredictorsIdx, vsPredictorsIdx, a_r_0, b_r_0, W_0,
            a_0, b_0, covariatesCorrelation, temperature, 0.25, nChains,
            nGlobalUpdates, accCountGlobalUpdates, countGlobalUpdates);
        break;
    }
...
void SEM_MCMC_step(
  ...
  // This function will take care of all the local AND global moves for all the
  // chains
#ifdef _OPENMP
...
#pragma omp parallel for num_threads(nThreads)
        ...
        for (unsigned int m = 0; m < nChains; ++m) {
          Model::MC3_SUR_MCMC_step(
              data, outcomesIdx[k], fixedPredictorsIdx[k], vsPredictorsIdx[k],
              omega_state[k].slice(m), gamma_state[k].slice(m),
              logPrior_state[k](m), logLik_state[k](m), a_r_0, b_r_0, W_0[k],
              a_0[k], b_0[k], accCount(k, m), nUpdates, temp[k](m));
...
#pragma omp parallel for num_threads(nThreads)
    ...
    if (nChains > 1) {
      Model::MCMC_Global_step(
          data, outcomesIdx[k], fixedPredictorsIdx[k], vsPredictorsIdx[k], k,
          omega_state[k], gamma_state[k], logPrior_state[k], logLik_state[k],
          a_r_0, b_r_0, W_0[k], a_0[k], b_0[k], parCrossOver,
          covariatesCorrelation[k], nChains, nGlobalUpdates,
          accCountGlobalUpdates[k], countGlobalUpdates[k], temp[k]);

      // ## Update temperature ladder
      if (Distributions::randIntUniform(0, 9) ==
          0)  // once every ten times on average
      {
        if ((accCountGlobalUpdates[k] / (double)countGlobalUpdates[k]) > 0.35) {
          temperatureRatio(k) += deltaTempRatio;
        } else {
          temperatureRatio(k) =
              std::max(1., temperatureRatio(k) - deltaTempRatio);
        }

        temperatureRatio(k) =
            std::min(temperatureRatio(k),
                     pow(maxTemperature, 1. / ((double)nChains - 1.)));
        ...
        for (unsigned int m = 1; m < nChains; ++m) {
          ...
          temp[k](m) =
              std::min(maxTemperature, temp[k](m - 1) * temperatureRatio(k));
...
/*
* IMPORTANT FOR THE FOLLOWING! REMEMBER IN OUR CASE WE HAVE BETAS THAT ARE DIFFERENT FROM
* THE OLS ESTIMATES, AND THUS THE DECOMPOSITION OF THE SS DOESN'T HOLD
* https://en.wikipedia.org/wiki/Explained_sum_of_squares#Partitioning_in_the_general_ordinary_least_squares_model
* WE NEED THUS TO USE THE GENERAL FORM OF THE COEFF OF DETERMINATION AS 1-resSS/totSS WHICH CAN BE NEGATIVE
