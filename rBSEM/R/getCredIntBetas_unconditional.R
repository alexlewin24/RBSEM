#' @title Get posterior credible intervals for regression coefficients (unconditional)
...
getCredIntBetasUnconditional <- function(blockGraph,blockList,outFilePath,outFilePrefix,varNames,
  ...
  for( iblock in 1:nblocks ){
    ...
    if( (1 %in% blockGraph[,iblock]) || (2 %in% blockGraph[,iblock]) ){
      ...
      #betaResultsTable <- abind(betaCredInt,betaMeans,along=2)
      ...
      print("Here means are not conditional on being selected. 0 if not selected.")
